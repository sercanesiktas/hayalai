...
`159 prompt`  ·  [← tüm kategoriler](../README.md#kategoriler)
...
> Düzenli ifade üretir. ChatGPT, Gemini veya Claude gibi sohbet asistanlarına yapıştırıp kullanırsın. Prompt metni İngilizce. Hazır katalog promptu.
...
- Real math (leave alone) — $\ightarrow$, $O(1)\	ext{ streaming}$ → valid, no warnings
...
- Verify the diff — the safety net: greps that must print nothing for real math ($\ightarrow$, \	ext) and shell vars ($HOME, $(…), ${VAR}). If anything legit was touched, it tells you to git checkout -- . and stops.
